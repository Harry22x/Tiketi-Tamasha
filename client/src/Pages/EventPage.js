...
function EventPage() {
 ...
 async function getevent(){
  try{const response = await fetch(`https://tiketi-tamashafrunt.onrender.com/events/${id}`)
 ...
 const handleTicketChange = (id, ticketType, price, change) => {
  ...
  async function redeucequantityavailable() {
    for (let ticket in selectedTickets) {
      try {
        const response = await fetch(`https://tiketi-tamashafrunt.onrender.com/event-tickets/${selectedTickets[ticket].id}`);
        const ticketData = await response.json();
  
        if (response.ok) {
          const newAvailableQuantity = ticketData.available_quantity - selectedTickets[ticket].quantity;
  
          await fetch(`https://tiketi-tamashafrunt.onrender.com/event-tickets/${selectedTickets[ticket].id}`, {
            method: "PATCH",
            headers: {
